...
export function createPublisher({ renderer, cache, storage, logger, events, crypto: cryptoApi = globalThis.crypto,
  ...
  async function publish(input) {
    ...
    const key = normalizePublicationKey(input.key); const publicationId = input.publicationId ?? `pub_${id()}`;
    ...
    try {
      ...
      const metadata = Object.freeze({ publicationId, correlationId, version, format: input.format, ...(input.metadata ?? {}) });
      try {
      ...
      if (!failures.length) {
        if (!await cache.set(manifestKey, { key, version, destination: input.destination, format: input.format }, { metadata })) failures.push('manifest');
        if (!await cache.invalidate(input.invalidateKey ?? key)) failures.push('invalidation');
      }
      if (events) try { await events.publish({ name: failures.length ? 'PublicationFailed' : 'ArtifactPublished', version: '1.0', source: 'core.publish', payload: { publicationId, key, version } }, { correlationId }); } catch (error) { failures.push('event'); logger.error('Publication event failed', { operation: 'publisher.event', correlationId, error }); }
