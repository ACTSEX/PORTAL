...
async function apiResponse(request, env, url) {
  ...
  try {
    ...
    if (url.pathname === '/api/me/media' && request.method === 'GET') {
      const identity = await app.auth.authenticate(request);
      const rows = await app.db.all("SELECT id, r2_key, mime_type, byte_size, sort_order FROM media WHERE owner_id = ? AND media_type = 'image' ORDER BY sort_order, created_at, id", [identity.user.id]);
    ...
    if (url.pathname === '/api/me/media' && request.method === 'POST') {
      ...
      const upload = createUpload({ storage, logger: app.logger, crypto, events: { async publish() {} }, registerMedia: async (input) => {
        ...
        return { id: mediaId, r2Key: input.r2Key, mimeType: input.mimeType, byteSize: input.byteSize, sortOrder: 0 };
      } });
...
export default {
  async fetch(request, env) {
    ...
    // Defense in depth: production routes only /api/*; a public pageview that is
    // accidentally sent here is rejected before runtime(), D1, R2 or Queue exist.
    if (!API_HOSTS.has(hostname) || !url.pathname.startsWith('/api/')) {
      return hardened(new Response('Not found', { status: 404, headers: { 'cache-control': 'no-store' } }), url);
    }
    return hardened(await apiResponse(request, env, url), url);
  },
