...
/** Create a fail-open technical cache backed by the ACTS_DATA R2 binding. */
export function createCache({ binding, logger, namespace = 'acts', visibility = 'private', defaultTtl = 300 }) {
  if (!binding || !['get', 'put', 'delete'].every((method) => typeof binding[method] === 'function')) throw new TypeError('Invalid R2 cache binding');
  ...
  async function get(key) {
    return safe('cache.get', Object.freeze({ hit: false, value: null, metadata: null }), async () => {
      const object = await binding.get(normalize(key));
      const stored = object && typeof object.text === 'function' ? await object.text() : object;
  ...
  async function set(key, value, { ttl = defaultTtl, metadata = {} } = {}) {
    ...
    return safe('cache.set', false, async () => { await binding.put(normalize(key), JSON.stringify({ value, metadata: { ...metadata, ttl } }), { httpMetadata: { contentType: 'application/json' } }); return true; });
  }
