...
function memoryCache() {
  const values = new Map(); const invalidated = [];
  return { values, async get(key) { return values.has(key) ? { hit: true, value: values.get(key).value } : { hit: false, value: null }; },
    invalidated, async set(key, value, options) { values.set(key, { value, options }); return true; },
    async invalidate(key) { invalidated.push(key); values.delete(key); return true; } };
}
...
test('publisher writes incrementally to KV, manifests and invalidates', async () => {
  ...
  const input = { destination: 'kv', format: 'html', key: '/Pages/Home', template: 'page',
    publicationId: 'pub_one', invalidateKeys: ['indexes/home'] };
  ...
  assert.equal(cache.values.get('pages/home').value, '<p>ok</p>');
  assert.deepEqual(cache.invalidated, ['indexes/home']);
...
test('publisher does not commit a manifest after invalidation failure and detects event rejection', async () => {
  const cache = memoryCache(); cache.invalidate = async () => false;
  const publisher = createPublisher({ renderer: { async render() { return '<p>new</p>'; } }, cache,
    storage: { async put() {} }, logger: logger(), events: { async publish() { return { failed: 1 }; } } });
  const result = await publisher.publish({ destination: 'kv', format: 'html', key: 'page',
    template: 'page', invalidateKeys: ['index'] });
  assert.equal(result.partial, true);
  assert.deepEqual(result.failures, ['invalidation', 'event']);
  assert.equal(cache.values.has('publications/page.manifest'), false);
  await assert.rejects(publisher.publish({ destination: 'kv', format: 'html', key: 'page',
    template: 'page', metadata: { secret: {} } }), /metadata/);
});
