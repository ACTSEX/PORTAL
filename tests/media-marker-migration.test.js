...
test('migração é idempotente se já removido e falha com outro objeto',async()=>{assert.equal((await migrate('apply',{env,remote:remote([])})).status,'ja_removido');await assert.rejects(migrate('apply',{env,remote:remote([MARKER_KEY,'midias/clientes/x/foto.webp'])}),/PREFIX_NOT_EMPTY/)});
