...
Registros exatos `www`, `dados`, `media` e apex têm precedência sobre `*`. Não criar `*.*`; `www.<slug>.acompanhantesex.com` não faz parte do produto e não se assume coberto pelo certificado `*.acompanhantesex.com`.
...
- match: `http.host eq "www.acompanhantesex.com"`;
- status: `308`;
- target dinâmico: `concat("https://acompanhantesex.com", http.request.uri.path)`;
...
|---|---|
| `https://www.acompanhantesex.com/` | `https://acompanhantesex.com/` |
| `https://www.acompanhantesex.com/cidade/sao-paulo` | `https://acompanhantesex.com/cidade/sao-paulo` |
| `https://www.acompanhantesex.com/cidade/sao-paulo?x=1` | `https://acompanhantesex.com/cidade/sao-paulo?x=1` |
| `https://www.acompanhantesex.com/api/me` | `https://acompanhantesex.com/api/me` |
...
- match: `http.host matches "^[a-z0-9]+(?:-[a-z0-9]+)*\\.acompanhantesex\\.com$"`;
- excluir hosts reservados: apex não casa; adicionar `not http.host in {"www.acompanhantesex.com" "dados.acompanhantesex.com" "media.acompanhantesex.com"}`;
...
Não é necessário nem recomendado interpolar o hostname em uma key R2. O navegador deriva o slug e solicita `/minisites/<slug>.json` diretamente.

## 5. R2 Custom Domains e CORS

- `dados.acompanhantesex.com` → `acts-dados`;
- `media.acompanhantesex.com` → `acts-midias`;
- desabilitar uso de `r2.dev` em produção;
- dados são públicos, sem cookies/credentials: permitir GET/HEAD de `https://acompanhantesex.com` e origens oficiais `https://<slug>.acompanhantesex.com`; se o mecanismo não aceitar wildcard de origem, `Access-Control-Allow-Origin: *` é aceitável somente para estes objetos deliberadamente públicos e sem credentials;
