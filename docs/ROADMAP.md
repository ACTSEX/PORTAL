...
- **Decisão anterior à implementação:** analisar especificamente schema e fluxo e definir cidade canônica, país, região, nome, slug, unicidade/normalização, vínculo e compatibilidade dos anúncios; versão monotônica e concorrência; digest/projeção e manifest vigentes; último estado, retry, lease/recuperação e reconciliação; índices, evolução e rollback. Avaliar ADR quando o modelo estiver definido.
- **Migration:** `database/migrations/0003_city_publication_state.sql` é somente nome candidato, sujeito à auditoria final. Não está autorizado nem planejado no TREE até existir modelo completo; nenhuma tabela excessiva ou migration preventiva é aprovada.
