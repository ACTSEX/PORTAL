...
A árvore possui **107 arquivos após o Lote 13B: 40 documentos e 66 não documentais**. Existem 3 migrations e 14 suítes de teste. A contagem não inclui `.git` nem caminhos planejados.
...
- `wrangler.toml` [E]
- `scripts/backfill-cities.js` [E]
- `tests/operations/city-backfill.test.js` [E]
...
| 13B | [E] `scripts/backfill-cities.js`, `tests/operations/city-backfill.test.js`; atualiza [E] `app/modules/Listings.js`, `wrangler.toml`, `package.json`, `package-lock.json` e testes diretamente afetados nos limites documentados. |
...
O executor `[E]` é o único adaptador Node operacional: usa `getPlatformProxy()` do Wrangler, o `wrangler.toml` raiz e o binding fixo `ACTS_DB`. Isso não autoriza arquivo de configuração, endpoint, Function, Worker, cliente REST, tabela de checkpoint ou helper adicional. `Listings.js` continua sendo o único local autorizado para a função canônica pública e impede novas escritas sem `city_id`.
...
O 13B está implementado localmente; sua execução real em staging permanece pendente. `0004`, `Publish.js`, `Seo.js` e todos os caminhos do Lote 14 continuam futuros e bloqueados.
