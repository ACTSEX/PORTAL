...
Em **GitHub → Actions → Corrigir estrutura de mídias → Run workflow**, selecione primeiro `plan`. Baixe o artifact `relatorio-correcao-midias-r2` e confirme que `objectsUnderPrefix` contém somente `midias/clientes/manifesto.json`, `deletes` é zero e o status é `marcador_vazio_confirmado`. Depois da aprovação do ambiente protegido `production-r2`, execute novamente escolhendo `apply`, na branch `main`, e preencha a confirmação `REMOVER-MARCADOR-MIDIAS-RAIZ`. Se o marcador já tiver sido removido, o workflow termina de forma idempotente com `ja_removido`. Qualquer outro objeto sob `midias/` bloqueia a operação e continua preservado.
