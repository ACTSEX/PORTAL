...
A árvore preserva o contrato: `clientes/{id}/dados/{perfil,midias}.json e clientes/{id}/site/configuracao.json`, `dados/cidades/{cidade}/{diretorio}/todos/{manifesto,NNN}.json`, categorias em `categorias/{categoria}/`, e ponteiros em `dados/sistema/`. Perfil/mídias não são duplicados por cidade ou categoria.
